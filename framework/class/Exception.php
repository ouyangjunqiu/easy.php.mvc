...
class Error
{
	...
	public static function exceptionError($exception)
	{
		...
		krsort($trace);
		$trace[] = array(
			'file' => $exception->getFile(),'line' => $exception->getLine(),'function' => 'break'
		);
		$phpMsg = array();
		foreach ($trace as $error)
		{
			...
			$phpMsg[] = array(
				'file' => str_replace(array(
					...
					'','/'
				), $error['file']),'line' => $error['line'],'function' => $error['function']
			);
		}
...
class DbException extends Exception
{
    ...
    /**
     * @var string $sql 当前执行的sql信息
     */
	protected $sql;

    /**
     * @var array $dbConfig 当前数据库配置信息
     */
	protected $dbConfig;
